...
import apps.accounts.management.commands.seed_testdata_once as seed_testdata_once_command
...
class SeedTestdataOnceCommandTest(TestCase):
    """Tests for seed_testdata_once management command."""

    def test_seeds_when_database_has_no_users(self):
        """Test command delegates to seed_testdata for an empty database."""
        with self.settings(DEBUG=True), patch.object(
            seed_testdata_once_command,
            "call_command",
        ) as mocked_call_command:
            stdout = StringIO()
            call_command("seed_testdata_once", stdout=stdout)

        mocked_call_command.assert_called_once()
        self.assertEqual(
            mocked_call_command.call_args.args,
            ("seed_testdata", "--config", "testdata_config_small.json"),
        )
        self.assertIn("No users found", stdout.getvalue())

    def test_skips_when_users_already_exist(self):
        """Test command does not reseed an existing database."""
        ...
        self.assertIn("Users already exist", stdout.getvalue())

    def test_rejects_non_debug_settings(self):
        """Test command is guarded against production-style settings."""
        with self.settings(DEBUG=False):
            with self.assertRaises(CommandError):
                call_command("seed_testdata_once", stdout=StringIO())

    def test_passes_custom_config_and_no_committees(self):
        """Test custom arguments are passed through to seed_testdata."""
        with self.settings(DEBUG=True), patch.object(
            seed_testdata_once_command,
            "call_command",
        ) as mocked_call_command:
            call_command(
                "seed_testdata_once",
                config="testdata_config.example.json",
                no_committees=True,
                stdout=StringIO(),
            )
