...
class ResolutionPermissionMixin:
    ...
    def get_user_permission(self, resolution, permission_codename):
        ...
        memberships = Membership.objects.filter(
            user=user, committee=resolution.committee, is_active=True
        ).select_related("role")
...
class ResolutionListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        return queryset.select_related("committee", "created_by").order_by(
            "-created_at"
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        if user.is_superuser or user.is_staff:
            committees = Committee.objects.filter(
                can_create_resolutions=True, is_active=True
            )
        else:
            memberships = Membership.objects.filter(
                user=user, is_active=True
            ...
            committees = Committee.objects.filter(
                id__in=committee_ids, can_create_resolutions=True, is_active=True
            )
...
class ResolutionDetailView(LoginRequiredMixin, ResolutionPermissionMixin, DetailView):
    ...
    def get_object(self, queryset=None):
        ...
        if not self.get_user_permission(resolution, "resolution.view"):
            raise PermissionDenied(
                "Sie haben keine Berechtigung diesen Beschluss anzusehen"
            )
    ...
    def get_context_data(self, **kwargs):
        ...
        context["can_edit"] = resolution.is_editable and self.get_user_permission(
            resolution, "resolution.edit"
        )
        context["can_delete"] = resolution.is_deletable and self.get_user_permission(
            resolution, "resolution.delete"
        )
        context["can_propose"] = (
            resolution.can_be_proposed
            and Resolution.user_can_propose(self.request.user, resolution.committee)
        )
        context["can_withdraw"] = (
            resolution.can_be_withdrawn
            and Resolution.user_can_propose(self.request.user, resolution.committee)
        )
...
class ResolutionCreateView(LoginRequiredMixin, CreateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not _user_has_agenda_permission(
            request.user, agenda, "agenda.add_item_resolution"
        ):
    ...
    def get_form(self, form_class=None):
        ...
        if agenda is not None:
            committee = agenda.meeting.committee
            form.fields["committee"].queryset = form.fields[
                "committee"
            ].queryset.filter(
                pk=committee.pk,
            )
...
class ResolutionUpdateView(LoginRequiredMixin, ResolutionPermissionMixin, UpdateView):
    ...
    def get_object(self, queryset=None):
        ...
        if not self.get_user_permission(resolution, "resolution.edit"):
            raise PermissionDenied(
                "Sie haben keine Berechtigung diesen Beschluss zu bearbeiten"
            )
    ...
    def form_valid(self, form):
        """Update resolution."""
        messages.success(self.request, "Beschluss wurde aktualisiert")
...
class ResolutionDeleteView(LoginRequiredMixin, ResolutionPermissionMixin, DeleteView):
    ...
    def get_object(self, queryset=None):
        ...
        if not self.get_user_permission(resolution, "resolution.delete"):
            raise PermissionDenied(
                "Sie haben keine Berechtigung diesen Beschluss zu löschen"
            )
...
class ResolutionStatusChangeView(
    LoginRequiredMixin, ResolutionPermissionMixin, DetailView
):
