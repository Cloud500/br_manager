    ...
    def record_result(
        *,
        ...
        if meeting.status != "IN_PROGRESS":
            raise ValidationError("Beschlussergebnisse können nur während einer laufenden Sitzung erfasst werden.")
        if meeting.current_agenda_item_id != agenda_item.pk:
            raise ValidationError("Beschlussergebnisse können nur für den aktuell aktiven TOP erfasst werden.")
        ...
        resolution = Resolution.objects.select_for_update().get(pk=resolution_agenda_item.resolution_id)
        ...
        resolution.decision_text = decision_text.strip()
        resolution.status = "APPROVED" if is_quorate and yes_votes > no_votes else "REJECTED"
        resolution.decided_at = timezone.now()
        resolution.save(update_fields=[
            "yes_votes",
            "no_votes",
            "abstentions",
            "is_quorate",
        ...
        data = {
            ...
            "decided_at": resolution.decided_at.isoformat() if resolution.decided_at else "",
        }
