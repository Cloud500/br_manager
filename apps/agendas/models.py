...
class Agenda(models.Model):
    ...
    def all_items(self) -> List['AgendaItem']:
        ...
        return list(self.items.select_related('parent').order_by('sort_order', 'created_at', 'id'))
    ...
    def recalculate_item_numbers(self) -> None:
        ...
        def calculate_number(item: 'AgendaItem') -> str:
            ...
            if item.id in visiting:
                raise ValidationError('Ungültige TOP-Hierarchie: zirkuläre Unterordnung ist nicht erlaubt.')
        ...
        if all_items:
            AgendaItem.objects.bulk_update(all_items, fields=['item_number'], batch_size=100)
...
class AgendaItem(models.Model):
    ...
    def clean(self) -> None:
        ...
        if self.parent_id:
            if self.parent_id == self.pk:
                raise ValidationError('Ein TOP kann sich nicht selbst untergeordnet werden.')
    ...
    def delete(self, *args, **kwargs):
        ...
        if self.has_published_election_descendant():
            raise ValidationError('TOPs mit veröffentlichten Wahlen können nicht gelöscht werden.')
        agenda = self.agenda
    ...
    def _is_published_election_changed(self) -> bool:
        ...
        if not original.is_published_election:
            return False
        fields = ['agenda_id', 'parent_id', 'title', 'description', 'sort_order', 'item_type']
