...
Das Sicherheitskonzept des BR Managers umfasst technische und organisatorische Maßnahmen zum Schutz der Vertraulichkeit,
Integrität und Verfügbarkeit aller verarbeiteten Daten. Die Maßnahmen orientieren sich an den Anforderungen der DSGVO,
des BetrVG sowie an bewährten Sicherheitsstandards (OWASP, BSI-Grundschutz).
...
- **Granulare Berechtigungen:** Das System verwendet feingranulare Permissions pro Modul (z. B. `meeting.*`,
  `todo.*`, `personnel.*`), die individuell pro Rolle und Gremium zugewiesen werden.
- **Besonders schützenswerte Daten:** Personelle Einzelmaßnahmen (Modul `personnel`) enthalten sensible Personaldaten.
  Alle regulären Gremiumsmitglieder sowie aktive Ersatzmitglieder haben Lesezugriff (`personnel.view`). Die Verwaltung
  (Erstellen, Bearbeiten) steht allen Mitgliedern in Ausschüssen zur Verfügung, bei denen das Modul aktiviert ist
  (`personnel_enabled`). Stellungnahmen können ausschließlich von Vorsitz und Stellv. Vorsitz erstellt werden.

### 8.3.2 Externe API-Absicherung

Der BR Manager stellt eine externe JSON-API bereit (`/personnel/api/create/`), über die z. B. die Personalabteilung
personelle Einzelmaßnahmen anlegen kann (siehe [05_api_konzept.md](05_api_konzept.md)).

| Maßnahme             | Umsetzung                                                           |
|----------------------|---------------------------------------------------------------------|
| Authentifizierung    | API-Key oder Token-basierte Authentifizierung (kein Cookie/Session) |
| Autorisierung        | Zugriff nur mit explizit freigeschaltetem API-Schlüssel pro Gremium |
| Rate Limiting        | Strikte Begrenzung der Anfragen pro API-Key                         |
| Input Validation     | Server-seitige Validierung aller JSON-Felder                        |
| Audit-Logging        | Jeder API-Aufruf wird vollständig protokolliert (inkl. Absender-IP) |
| Netzwerkbeschränkung | IP-Whitelisting für API-Zugriff                                     |

### 8.3.3 Request-Sicherheit
...
|-------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **A01 – Broken Access Control**     | RBAC mit granularen Permissions pro Modul (`meeting.*`, `todo.*`, `personnel.*`), Objekt-Level-Berechtigungen, Audit-Logging, UUIDs als Primärschlüssel in allen URLs (IDOR-Schutz) |
...
- **Lock-Files:** `uv.lock` mit gepinnten Versionen (Paketverwaltung über uv).
...
| **Sitzungen**                  | Erstellen, Bearbeiten, Löschen, Abschließen, Versand von Einladungen                    |
| **Beschlüsse**                 | Abstimmungen, Wahlergebnisse                                                            |
| **To-Dos**                     | Erstellen, Bearbeiten, Zuweisen, Löschen, Statusänderungen                              |
| **Personelle Einzelmaßnahmen** | Erfassung, Bearbeitung, Friständerungen, Stellungnahmen, Ergebnisversand an Arbeitgeber |
| **Externe API-Zugriffe**       | Alle Aufrufe der externen API (personnel), inkl. Absender-IP und API-Key                |
